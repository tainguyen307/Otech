...
function App() {
  ...
  return <div className="min-h-screen bg-[#f5f7fa] text-[#172238] dark:bg-slate-950 dark:text-slate-100">
    ...
    <Header onAuth={requireAuth} darkMode={darkMode} onToggleTheme={() => setDarkMode((value) => !value)} user={user} onSignOut={() => { signOut(); setActiveNav('Home') }} onHome={() => setActiveNav('Home')} onProfile={() => setActiveNav('Profile')} />
    ...
    {activeNav === 'Marketplace' && <HomeScreen liked={liked} saved={saved} onLike={(id) => toggle(liked, setLiked, id)} onSave={(id) => toggle(saved, setSaved, id)} onAuth={requireAuth} />}
      {activeNav === 'Profile' && user && <ProfileScreen user={user} onUpdateUser={updateUser} onSignOut={() => { signOut(); setActiveNav('Home') }} />}
